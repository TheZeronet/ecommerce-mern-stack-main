...
export default function Cart() {
  ...
  const checkout = async () => {
    const res = await axios.post(
      "https://gvk-ecommerce-api.onrender.com/api/v2/ecommerce/cart/checkout",
      { user: user }
    );
    console.log(res);
  ...
  const displayPro = async () => {
    const res = await axios.post(
      "http://localhost:9002/api/v2/ecommerce/displayCart",
      { user: user }
    );
    if (res.data.success === true) {
  ...
  const removeProduct = async (productId, event) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:9002/api/v2/ecommerce/cart/removeProduct",
        { productId }
      );
              ...
              <div className="prices">
                ...
                <div className="remove">
                  <NavLink
                    to="#"
                    style={{ color: "red" }}
                    onClick={(event) => removeProduct(product._id, event)}
                  >
                    Remove
                  </NavLink>
                </div>
